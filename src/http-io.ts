...
import { createChunkedEncodingStream, createChunkedDecodingStream, connectionToReadableStream } from './streams.ts';
import { UnexpectedEofError } from './errors.ts';
import { createBodyParser } from './body-parser.ts';
import { TimeoutOptions, ResponseWithExtras } from './types.ts';
...
export async function writeRequest(
  ...
  // Write request line
  const requestLine = `${method.toUpperCase()} ${url.pathname}${url.search || ""} HTTP/1.1\r\n`;
...
export async function readResponse(
  ...
  if (isChunked) {
    ...
    let bytesRead = 0;
    bodyStream = bodyStream.pipeThrough(new TransformStream({
      transform(chunk, controller) {
        ...
        const toEnqueue = chunk.slice(0, Math.min(chunk.length, remaining));
  ...
  const bodyParser = createBodyParser(bodyStream, headers.get("content-type") ?? "");
